...
## always recommends the top-N on average most popular items
REAL_POPULAR <- function(data, parameter = NULL) {
  p <- .get_parameters(list(
      normalize = "center",
      aggregation = colMeans
		), parameter)
  ...
  model <- c(list(
    ratings = new("realRatingMatrix", data = dropNA(t(p$aggregation(data))),
      normalize = data@normalize)), p)
  ...
  predict <- function(model, newdata, n = 10,
    ...
    ## denormalize ratings and produce a rating row for each user in newdata
    ratings <- denormalize(model$ratings)
    ratings <- ratings[rep(1L, nrow(newdata)),]

    ## helper function to remove known items and
    ## produce ratings matrices and topN-lists
