...
- coercion data.frame -\> realRatingMatrix: added drop = TRUE so it
  works with tidyverse tibbles (reported by youngroklee-ml)
...
- We require now proxy version \>= 0.4-26 which fixed a conversion bug
...
- Recommender gained method “HYBRID” to create hybrid recommenders. Now
  hybrid recommenders can also be used in evaluate().
...
- Recommender UBCF now removes self-matches if item ids are specified in
  newdata. Specifying data in predict is no longer necessary. (reported
  by aliko-str).
...
- predict with type “ratingMatrix” now returns predictions for the known
  ratings instead of replacing them with the known values.
- Recommender methods Popular, AR and RERECOMMENDER now also return
  ratings for binary data (and thus can be used for HybridRecommender).
...
- Fixed bug in denormalization by column with z-score (reported by
  jackyrx).
- Fixed bug in predict with type “ratingMatrix” where known values were
  not denormalized (reported by MounirHader).
...
- Fixed bug in method IBCF with n being ignored in predict (reported by
...
- predict now checks if newdata (number of items) is compatible with the
  model.  
