...
REAL_LIBMF <- function(data, parameter = NULL) {
  ...
  dat <- as(rbind(as(model$data, "dgCMatrix"), as(newdata, "dgCMatrix")), "dgTMatrix")

  r <- Reco()
  r$train(data_memory(dat@i, dat@j, dat@x),
    opts = list(dim = p$dim, costp_l2 = p$costp_l2, costq_l2 = p$costq_l2, nthread = p$nthread)
  )
  ...
  ratings <- new("realRatingMatrix", data = as(new("dgTMatrix", i = i-nrow(model$data),
    j = j, x = x, Dim = newdata@data@Dim, Dimnames = newdata@data@Dimnames), "dgCMatrix"))

  returnRatings(ratings, newdata, type, n)
  }

  ## construct recommender object
  new("Recommender", method = "LIBFM", dataType = class(data),
...
recommenderRegistry$set_entry(
  method="LIBMF", dataType = "realRatingMatrix", fun=REAL_LIBMF,
